...
parser.add_argument('--gpu', default='0', type=str,
                    help='GPU ID')
parser.add_argument('--resume', default='/home/liu/chenhaoran/Mymodel/record823/1101checkpoint_epoch28.pth', type=str, metavar='PATH',
...
writer = SummaryWriter(
    'runs/' + '1108_%d-%d_tensorboard' % (datetime.datetime.now().month, datetime.datetime.now().day))
...
def main():
    ...
    for epoch in range(args.start_epoch, args.maxepoch):
        ...
        """
        output_name = 'checkpoint%d-stage1-%f-f1%f-precision%f-acc%f-recall%f.pth' % (epoch,val_avg['loss_avg'],val_avg['f1_avg'],
    ...
    for batch_index, (images, labels_numpy) in enumerate(generate_minibatches(dataParser, True)):
        ...
        with torch.set_grad_enabled(True):
            ...
            if not args.band_mode:
                ...
                loss = cross_entropy_loss(outputs[0], labels[0]) * args.fuse_loss_weight
                ...
                loss = cross_entropy_loss(outputs[0], labels[0])
...
def val(model, dataParser, epoch):
    ...
    for batch_index, (images, labels_numpy) in enumerate(generate_minibatches(dataParser, False)):
        ...
        if not args.band_mode:
            # 如果不是使用band_mode 则需要计算8张图的loss
            loss = wce_huber_loss(outputs[0], labels[0]) * args.fuse_loss_weight
            ...
            loss = wce_huber_loss(outputs[0], labels[0])
