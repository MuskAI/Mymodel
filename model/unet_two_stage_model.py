...
class UNetStage1(nn.Module):
    ...
    def forward(self, x):
        ...
        return [logits, stage_x1, stage_x2, stage_x3, stage_x4]
...
class UNetStage2(nn.Module):
    def __init__(self, n_channels=4, bilinear=True):
        ...
        self.fuse2 = FuseStageOut(in_channels=128 + 64, out_channles=64)
        self.fuse3 = FuseStageOut(in_channels=256 + 128, out_channles=128)
        self.fuse4 = FuseStageOut(in_channels=512 + 256, out_channles=256)
    ...
    def forward(self, x, stage2, stage3, stage4):
        ...
        # fuse stage
        x2 = self.maxpool(x1)
        x2 = self.fuse2(x2, stage2)
        x2 = self.down1(x2)

        x3 = self.maxpool(x2)
        x3 = self.fuse3(x3, stage3)
        x3 = self.down2(x3)

        x4 = self.maxpool(x3)
        x4 = self.fuse4(x4, stage4)
        x4 = self.down3(x4)
...
if __name__ == '__main__':
    ...
    # summary(model, [(4, in_size, in_size), (128, in_size // 2, in_size // 2), (256, in_size // 4, in_size // 4),
