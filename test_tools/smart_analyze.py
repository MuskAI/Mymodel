...
time:12/26
description:
1 generate analyze reporter
2.you only need to input pred dir, src dir, gt_dir
3.the reporter contains img, excel, easy_example, hard_example
4.when you implement analyze, you only need to override __find_src_and_gt
"""
...
class Analyze:
    def __init__(self):
        ...
        self.combine_dir = '/media/liu/File/10月数据准备/1108_数据测试/cm_train_data/combineImg'
    ...
    def __path_check(self):
        if not os.path.exists(self.pred_dir):
            print('The pred dir error!')
            exit(0)
        elif not os.path.exists(self.src_dir):
            print('The src dir error!')
            exit(0)
        elif not os.path.exists(self.gt_dir):
            print('The gt dir error!')
            exit(0)
        else:
            pass
...
class Coverage(Analyze):
    def __init__(self):
        super().__init__()
    def __find_src_and_gt(self):
        pass
