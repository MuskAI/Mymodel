...
"""
created by xl, modified by haoran
time 11/05
des:
1. 对band result 进行分析
2. 通过loss值、f1
...
class Analyze():
    def __init__(self):
        self.pred_dir = '/media/liu/File/11月数据准备/AFTER_320_CROP_COVERAGE_DATASET/pred/1130stage_2_pred'
        self.src_dir = '/media/liu/File/11月数据准备/AFTER_320_CROP_COVERAGE_DATASET/src'
        self.gt_dir = '/media/liu/File/11月数据准备/AFTER_320_CROP_COVERAGE_DATASET/gt'

        self.band_dir = '/media/liu/File/11月数据准备/coverage_test_reporter/edge'
        self.combine_dir = '/media/liu/File/11月数据准备/coverage_test_reporter/combineImg2'
        self.save_excel_dir = '/media/liu/File/11月数据准备/coverage_test_reporter/coverage_edge_test.xlsx'
    def analyze_all(self):
        ...
        for index, name in enumerate(sample):
            ...
            loss_tonsor = wce_huber_loss(pred_img_tensor.float(), band_gt_tensor.float())
    ...
    def __find_src_and_gt(self, name):
        ...
        gt_name = src_name.replace('.bmp','_gt.bmp')
