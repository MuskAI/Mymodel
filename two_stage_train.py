...
def train(model1,model2, optimizer1,optimizer2, dataParser, epoch, save_dir):
        ...
        with torch.set_grad_enabled(True):
            ...
            if abs(batch_index - dataParser.steps_per_epoch) % mid_freq == 0:
                for i in range(args.batch_size):
                    ...
                    t = t.transpose(1,2,0)
                    t = Image.fromarray(t)
                    t.save('mid_result/two_stage_input/two_stage_the_midinput_%d_%d.png'%(epoch,batch_index))
            ...
            if abs(batch_index - dataParser.steps_per_epoch) % mid_freq == 0:
                for i in range(args.batch_size):
                    ...
                    t = t.transpose(1,2,0)
                    t = Image.fromarray(t)
                    t.save('mid_result/one_stage_input/two_stage_the_midinput_%d_%d.png'%(epoch,batch_index))
            two_stage_outputs = model2(rgb_pred,one_stage_outputs[9],one_stage_outputs[10],one_stage_outputs[11])
            ...
            if False:
                ...
                for c_index, c in enumerate(one_stage_outputs[1:9]):
                    ...
                    loss_8t += one_loss_t
                    writer.add_scalar('stage1_%d_map_loss'%(c_index),one_loss_t.item(),global_step=epoch * train_epoch + batch_index)
            ...
            _loss_stage_2 = wce_huber_loss(two_stage_outputs[0], labels[0]) * 12
            ...
            if abs(batch_index - dataParser.steps_per_epoch) % mid_freq == 0:
                for i in range(args.batch_size):
                    ...
                    t.save('mid_result/two_stage_output/two_stage_the_midoutput_%d_%d.png'%(epoch,batch_index))
            ...
            if abs(batch_index - dataParser.steps_per_epoch) % mid_freq == 0:
                for i in range(args.batch_size):
                    ...
                    t.save('mid_result/one_stage_output/one_stage_the_midoutput_%d_%d.png'%(epoch,batch_index))
            # 这里放保存中间结1果的代码
    ...
    # 保存模型
    if epoch%1 == 0:
        save_file = os.path.join(args.model_save_dir, '1104_stage1_checkpoint_epoch{}.pth'.format(epoch))
        save_checkpoint({'epoch': epoch, 'state_dict': model1.state_dict(), 'optimizer': optimizer1.state_dict()},
                        filename=save_file)
        save_file = os.path.join(args.model_save_dir, '1104_stage2_checkpoint_epoch{}.pth'.format(epoch))
        save_checkpoint({'epoch': epoch, 'state_dict': model2.state_dict(), 'optimizer': optimizer2.state_dict()},
                        filename=save_file)
    # save_checkpoint({
    #     'epoch': epoch,
