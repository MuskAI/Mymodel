...
def main():
    ...
    for epoch in range(args.start_epoch, args.maxepoch):
        train_avg = train(model1=model1, optimizer1=optimizer1,
                          dataParser=trainDataLoader, epoch=epoch)

        val_avg = val(model1=model1, dataParser=valDataLoader, epoch=epoch)
        test(model1=model1, dataParser=testDataLoader, epoch=epoch)

        """"""""""""""""""""""""""""""
        "          写入图             "
        """"""""""""""""""""""""""""""
        try:
            writer.add_scalars('lr_per_epoch', {'stage1': scheduler1.get_lr()}, global_step=epoch)
...
@torch.no_grad()
def val(model1, dataParser, epoch):
    # 读取数据的迭代器
