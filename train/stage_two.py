...
parser.add_argument('--gpu', default='0', type=str,
                    help='GPU ID')
parser.add_argument('--resume', default=['/home/liu/chenhaoran/Mymodel/save_model/stage1_0119_template_cod10k_cm_sp_negative_texture_blur_train/0119_template_sp_negative_COD10K_texture_checkpoint20-stage1-0.298484-f10.609795-precision0.904922-acc0.971729-recall0.479262.pth',
                                         '/home/liu/chenhaoran/Mymodel/save_model/1227_model_two_stage_band5_texture_data/1119checkpoint20-stage2-0.089925-f10.729159-precision0.951136-acc0.995505-recall0.632400.pth'], type=list, metavar='PATH',
...
writer = SummaryWriter(
    'runs/' + '0122_1-22_tensorboard')

output_name_file_name = '0122_template_sp_negative_COD10K_texture_checkpoint%d-two_stage-%f-f1%f-precision%f-acc%f-recall%f.pth'
""""""""""""""""""""""""""""""
"    ↑↑↑↑需要修改的参数↑↑↑↑     "
""""""""""""""""""""""""""""""
...
def main():
    ...
    for epoch in range(args.start_epoch, args.maxepoch):
        train_avg = train(model1=model1, model2=model2, optimizer1=optimizer1, optimizer2=optimizer2, dataParser=trainDataLoader, epoch=epoch)
        print('start val')
        ...
        """"""""""""""""""""""""""""""
        try:
            writer.add_scalar('tr_avg_loss_per_epoch', train_avg['loss_avg_stage2'], global_step=epoch)
            writer.add_scalar('tr_avg_f1_per_epoch', train_avg['f1_avg_stage2'], global_step=epoch)
            writer.add_scalar('tr_avg_precision_per_epoch', train_avg['precision_avg_stage2'], global_step=epoch)
            writer.add_scalar('tr_avg_acc_per_epoch', train_avg['accuracy_avg_stage2'], global_step=epoch)
            writer.add_scalar('tr_avg_recall_per_epoch', train_avg['recall_avg_stage2'], global_step=epoch)
            ...
            writer.add_scalar('lr_per_epoch_stage1', scheduler1.get_lr(), global_step=epoch)
        ...
        if epoch % 1 == 0:
            ...
            torch.save({'epoch': epoch, 'state_dict': model1.state_dict(), 'optimizer': optimizer2.state_dict()},
...
def train(model1, model2, optimizer1, optimizer2, dataParser, epoch):
    ...
    for batch_index, input_data in enumerate(dataParser):
        ...
        writer.add_scalar('precision_score_stage1', precisionscore_stage1, global_step=epoch * train_epoch + batch_index)
...
def val(model1, model2,dataParser, epoch):
    ...
    for batch_index, input_data in enumerate(dataParser):
        ...
        with torch.set_grad_enabled(False):
            ...
            for c_index, c in enumerate(two_stage_outputs[1:9]):
                one_loss_t = wce_dice_huber_loss(c, relation_map[c_index].cuda())
                loss_8t += one_loss_t
                writer.add_scalar('stage2_%d_map_loss' % (c_index), one_loss_t.item(),
            ...
            writer.add_scalar('val_stage_two_pred_loss', _loss_stage_2.item(), global_step=epoch * val_epoch + batch_index)
            writer.add_scalar('val_stage_two_fuse_loss', loss_stage_2.item(), global_step=epoch * val_epoch + batch_index)
        ...
        writer.add_scalar('val_precision_score_stage1', precisionscore_stage1, global_step=epoch * val_epoch + batch_index)
...
@torch.no_grad()
def test(model1, model2, optimizer1, optimizer2, dataParser, epoch):
    # 读取数据的迭代器
    ...
    for batch_index, input_data in enumerate(dataParser):
        ...
        with torch.set_grad_enabled(False):
            ...
            writer.add_scalar('test_stage_two_pred_loss', _loss_stage_2.item(), global_step=epoch * test_epoch + batch_index)
            writer.add_scalar('test_stage_two_fuse_loss', loss_stage_2.item(), global_step=epoch * test_epoch + batch_index)
        ...
        writer.add_scalar('test_precision_score_stage1', precisionscore_stage1, global_step=epoch * test_epoch + batch_index)
